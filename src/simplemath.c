...
/* 
   ...
   approximation of trigonometric functions & sqrt
   followed link & instructions from https://github.com/ewedel/pebble-sunclock/blob/master/src/my_math.c
*/

float simple_floor(const float x) 
{
  return ((int)x);  
}

float simple_fabs(const float x)
{
  return x < 0 ? -x : x;
}

/* not quite rint(), i.e. results not properly rounded to nearest-or-even */
float simple_rint(const float x)
{
  float t = simple_floor(simple_fabs(x) + 0.5);
  return (x < 0.0) ? -t : t;
}

/* minimax approximation to cos on [-pi/4, pi/4] with rel. err. ~= 7.5e-13 */
float cos_core(const float x)
{
  float x8, x4, x2;
  x2 = x * x;
  x4 = x2 * x2;
...
float simple_sin(const float x)
{
  float q, t;
  int quadrant;
  /* Cody-Waite style argument reduction */
  q = simple_rint(x * 6.3661977236758138e-1);
  quadrant = (int)q;
...
float simple_cos(const float x)
{
  return simple_sin(x + (M_PI/2));
}

/* relative error < 2e-11 on [-1, 1] */
float simple_acos(const float x)
{
  float xa, t;
  xa = simple_fabs(x);
   ...
   */
  if (xa > 0.5625) {
    t = 2.0 * asin_core(simple_sqrt(0.5 * (1.0 - xa)));
  } else {
...
/* substrace acos from PI/2 to get asin */
float simple_asin(float x)
{
  return (M_PI/2) - simple_acos(x);
}

/* tan = sin / cos */
float simple_tan(const float x)
{
